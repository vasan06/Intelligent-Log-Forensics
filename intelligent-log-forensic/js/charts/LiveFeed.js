...
class LiveFeedTable {
  ...
  renderRow(log, animate = true) {
    ...
    row.innerHTML = `
      ...
      <td class="forensic-mono" style="color: #0891B2; font-weight: 600;">${this.escape(log.source_ip)}</td>
      ...
      <td class="forensic-mono" style="max-width: 280px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${this.escape(log.payload_snippet || log.raw_message || '')}">
        <span style="color: #4F46E5; font-weight: 600;">${this.escape(log.method || 'GET')}</span> ${this.escape(log.target || '/')} — ${this.escape(log.payload_snippet || '')}
      </td>
      <td class="forensic-mono" style="font-weight: 600; color: ${log.confidence_score >= 80 ? '#DC2626' : '#0891B2'};">
