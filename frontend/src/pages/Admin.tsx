...
import {
  Activity, CheckCircle, Cpu, Database,
  Play, Server, Shield, Square, Users,
} from "lucide-react";
    ...
    <div className="page">
      <div className="page-header">
        <div className="page-title">
          ...
          <p>System overview, user management, and forensic evidence audit.</p>
        </div>
        {overview.data && (
          <Badge tone={overview.data.generator.running ? "ok" : "default"}>
      ...
      {overview.loading ? (
        <div className="grid-4">
          {Array.from({ length: 4 }).map((_, i) => <div key={i} className="stat-card"><Skeleton /></div>)}
        </div>
      ) : overview.error ? (
        <Card>
          <EmptyState title="Access denied" hint="Administrator role required." icon={<Shield size={32} strokeWidth={1.4} />} />
        </Card>
      ) : overview.data ? (
        <>
          <div className="grid-4">
            <StatCard label="Total users"    value={overview.data.users}    icon={<Users size={14} />} />
            <StatCard label="Evidence files" value={overview.data.uploads}  icon={<Server size={14} />} />
            <StatCard label="Log records"    value={overview.data.logs}     icon={<Database size={14} />} />
            <StatCard label="Risk events"    value={overview.data.risks}    icon={<Activity size={14} />} tone={(overview.data.risks ?? 0) > 0 ? "medium" : "default"} />
          </div>
  ...
  async function toggleGenerator() {
    ...
    try {
      if (overview.generator.running) {
        ...
        toast("Generator stopped", "info");
      } else {
        await api.generatorStart("normal");
        toast("Generator started in normal mode", "success");
      }
      onRefresh();
    } catch (err) {
      toast(err instanceof Error ? err.message : "Generator action failed", "error");
    } finally { setBusy(false); }
  }

  return (
    <Card title={<><Cpu size={14} /> Live Generator &amp; Model</>}>
      <div className="flex gap-6 flex-wrap items-center">
        <div className="kv-list" style={{ flex: 1, minWidth: 200 }}>
          <div className="kv-item">
            <span className="kv-key">Generator status</span>
            <Badge tone={overview.generator.running ? "ok" : "default"}>
              {overview.generator.running ? "running" : "stopped"}
            </Badge>
          </div>
          <div className="kv-item">
            <span className="kv-key">Current mode</span>
            <span className="kv-value">{overview.generator.mode || "—"}</span>
          </div>
          <div className="kv-item">
            <span className="kv-key">Model version</span>
            <span className="kv-value mono" style={{ fontSize: "var(--text-xs)" }}>{overview.model_version ?? "—"}</span>
          </div>
          {overview.generator.last_error && (
            <div className="kv-item">
              <span className="kv-key">Last error</span>
              <span className="kv-value" style={{ color: "var(--critical)" }}>{overview.generator.last_error}</span>
            </div>
          )}
        </div>
        <Button
...
function UsersTab() {
  ...
  if (!users.data || users.data.length === 0) return <EmptyState title="No users" />;
  return (
    <div className="table-wrap">
      <table className="table">
        <thead>
          <tr><th>Name</th><th>Email</th><th>Role</th><th>Created</th><th>Last login</th><th>Status</th></tr>
        </thead>
        <tbody>
          {users.data.map((u) => (
            <tr key={u.id}>
...
function UploadsTab() {
  ...
  if (!uploads.data || uploads.data.length === 0) return <EmptyState title="No uploads" />;
  return (
    <div className="table-wrap">
      <table className="table">
        <thead>
          <tr><th>File</th><th>Type</th><th>Owner</th><th>Uploaded</th><th>Records</th><th>Status</th></tr>
        </thead>
        <tbody>
          {uploads.data.map((u) => (
            <tr key={u.id}>
...
function DatabaseTab() {
  ...
  return (
    <div className="kv-list" style={{ maxWidth: 400 }}>
      ...
      <div className="kv-item"><span className="kv-key">Version</span><span className="kv-value mono" style={{ fontSize: "var(--text-xs)" }}>{db.data.version}</span></div>
