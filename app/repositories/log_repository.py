...
def for_file(file_id, status=None, search=None, source_type=None, limit=500):
    ...
    if status is not None: clauses.append("status_code=%s"); params.append(status)
    if search: clauses.append("(raw_log ILIKE %s OR endpoint ILIKE %s)"); params.extend([f"%{search}%", f"%{search}%"])
    if source_type: clauses.append("log_source_type=%s"); params.append(source_type)
    params.append(limit)
